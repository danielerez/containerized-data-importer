...
func (c *SmartCloneController) processSnapshotItem(snapshot *cdisnapshotsv1.VolumeSnapshot) error {
	...
	sourcePvc, err := c.pvcLister.PersistentVolumeClaims(snapshot.Namespace).Get(snapshot.Spec.Source.Name)
	klog.V(3).Infof("!!! smart-clone processSnapshotItem sourcePvc name: %s", sourcePvc.Name)
	if err != nil {
		return err
	}
	newPvc := newPvcFromSnapshot(snapshot, sourcePvc)
	if newPvc == nil {
...
func newPvcFromSnapshot(snapshot *cdisnapshotsv1.VolumeSnapshot, sourcePvc *corev1.PersistentVolumeClaim) *corev1.PersistentVolumeClaim {
